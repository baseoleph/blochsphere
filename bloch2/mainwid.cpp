...
// ------------------------------
{
    ...
    connect(qV, SIGNAL(alpBetChanged(float, comp_d)), SLOT(slotAlpBetChanged(float, comp_d)));
    connect(qV, SIGNAL(xyzChanged(float, float, float)), SLOT(slotXYZChanged(float, float, float)));
    connect(qV, SIGNAL(newOp(QUOperator &)), SLOT(slotNewOp(QUOperator &)));
    qV->report();
}
...
// -------------------------------
{
    ...
    case 0:
        ...
        v = gamma / 2.0;
        curOperator = QUOperator(exp(C_I * (alpha - beta / 2.0 - delta / 2.0)) * cos(v),
    ...
    case 2:
        ...
        curOperator = QUOperator(exp(C_I * alpha) * (cos(v) * cos(w) - C_I * sin(v) * sin(u)),
...
// -----------------
{
    ...
    axRnEd->setText(
        QString("%1;%2;%3").arg(okr(x[0], 1000)).arg(okr(x[1], 1000)).arg(okr(x[2], 1000))); // 4
...
OpItem::OpItem(QString str, QUOperator op) : QListWidgetItem(str), oper(op) {
...
// ----------------------------------
{
    ...
    if (str.contains(rxp1)) {
        if (rxp1.capturedTexts()[2] == "+")
            ...
            return comp_d(rxp1.capturedTexts()[1].toDouble(), rxp1.capturedTexts()[2].toDouble());
