...
QUOperator::QUOperator(comp_d a_,
                       comp_d b_,
                       comp_d c_,
                       comp_d d_)
// Конструктор
...
{    
    if (abs(mA)&&abs(mB))
    {
        ...
        comp_d z=-2.0*mA*mB*
                 exp(-C_I*(2.0*alpha-beta));
        gamma=real(boost::math::asin(z));
...
// -------------------
{
    if (abs(mA)&&abs(mB))
    {
        ...
        comp_d z=-2.0*mA*mB*
                 exp(-C_I*(M_PI/2.0+2.0*alpha-beta));
        gamma=real(boost::math::asin(z));
    ...
    double v=gamma/2.0;
    if (
        abs(exp(C_I*(alpha-beta/2.0-delta/2.0))*cos(v)-mA)>EPS||
...
// -------------------
{
    ...
    if (fabs(a1)<EPS&&fabs(b2)<EPS)
	    if (fabs(b1)>EPS)
	    {
	        beta=2.0*atan(a2/b1); gamma=M_PI;
	        delta=0;         // delta - любое вещественное число
	    }
	    else if (fabs(a2)>EPS)
	         {
	             beta=M_PI-2.0*atan(b1/a2); gamma=M_PI;
	             delta=0;    // delta - любое вещественное число
	         }
    // ----------------------------
    if (fabs(a2)<EPS&&fabs(b1)<EPS)
	    if (abs(a1)>EPS)
	    {
	        beta=2.0*atan(-b2/a1); gamma=0;
	        delta=0;         // delta - любое вещественное число
	    }
	    else if (fabs(b2)>EPS)
	         {
	             beta=M_PI-2.0*atan(-a1/b2); gamma=0;
	             delta=0;    // delta - любое вещественное число
	         }
    // --------------------------------------------------------
    if (fabs(a1)<EPS&&fabs(b1)<EPS&&fabs(a2)>EPS&&fabs(b2)>EPS)
    {
	    beta=M_PI; gamma=-2.0*asin(a2); delta=0;
    }
    // --------------------------------------------------------
    if (fabs(a1)>EPS&&fabs(b1)>EPS&&fabs(a2)<EPS&&fabs(b2)<EPS)
    {
	    beta=0; gamma=-2.0*asin(b1); delta=0;
    }
    // --------------------------------------------------------
    if (fabs(a1)>EPS&&fabs(a2)>EPS&&fabs(b1)<EPS&&fabs(b2)<EPS)
    {
	    beta=M_PI/2.0; gamma=2.0*asin(-a2); delta=-M_PI/2.0;
    }
    // --------------------------------------------------------
    if (fabs(a1)>EPS&&fabs(a2)>EPS&&fabs(b1)>EPS&&fabs(b2)>EPS)
    {
...
// -----------------------
{
    comp_d cA = std::conj(mA),
           cB = std::conj(mC),
           cC = std::conj(mB),
           cD = std::conj(mD);
    return eq((mA*cA+mB*cC), comp_d(1,0))&&
           eq((mA*cB+mB*cD), comp_d(0,0))&&
           eq((mC*cA+mD*cC), comp_d(0,0))&&
           eq((mC*cB+mD*cD), comp_d(1,0));
}

QUOperator QUOperator::operator+(QUOperator &op) const
{
    return QUOperator(mA+op._a(),
                      mB+op._b(),
                      mC+op._c(),
                      mD+op._d());
}
...
{
    QVector<double> qv;
    double th=2.0*acos(cos(gamma/2.0)*cos(beta/2.0+delta/2.0)),
           sint=sin(th/2.0),
...
{
    the/=2.0;
    return QUOperator(cos(the),    -C_I*sin(the),
                     -C_I*sin(the), cos(the));
}
...
{  
    the/=2.0;
    return QUOperator(cos(the), -sin(the),
                      sin(the),  cos(the));
}
...
{
    the/=2.0;
    return QUOperator(exp(-C_I*the), 0.0,
                      0.0,   exp(C_I*the));
}
