...
void QBVector::rotation(float ng,rotType rt,
                        float nX, float nY, float nZ)
// -----------------------------------------
{
    switch(rt)
    {
    case rX: rotateX(ng); break;
    case rY: rotateY(ng); break;
    case rZ: rotateZ(ng); break;
    case rN: applyQUOp(cos(ng/2.0)*Iop-C_I*sin(ng/2.0)*(nY*Yop+nX*Xop+nZ*Zop));
             break;
    }
    report();
}
...
// -----------
    : QGLWidget(parent)
{
    xRot=-60; yRot=0; zRot=-135; nSca=1.2;
    sRadius=1; iter=0; naX=0;naY=0;naZ=0; trCol=0;
    oZCol=Qt::black; oYCol=Qt::black; oXCol=Qt::black;
    isTimerActive=false; showNewAx=false; traceFlag=true;
...
void Bloch::showDecC()
{
    ...
    QString st = QString("(%1;%2;%3)").arg(okr(qubitVector._x(),100))  //%4
...
void Bloch::drawNewAx()
{
    ...
    cond = !((abs(ax)>0.001f||abs(ay)>0.001f)
           &&(abs(ax)>0.001f||abs(az)>0.001f)
    ...
    glBegin(GL_LINES);
        for(int i=tracePts.size()-1;i>5;i-=3)
        {
            ...
            else
            {
                ...
                {
                    glVertex3f(tracePts[i-2],
                               tracePts[i-1],
                               tracePts[i]);
                    glVertex3f(tracePts[i-5],
                               tracePts[i-4],
                               tracePts[i-3]);
                }
            }
...
// -----------------
{
    ...
    if(abs(iter)<abs(rZYCur.ang))
    {
        ...
        if(rZYCur.ang>0)
        {
            ...
            if(rZYCur.typ==rN)
                qubitVector.rotation(RAD,rZYCur.typ,rZYCur.naX,
                                                    rZYCur.naY,
                                                    rZYCur.naZ);
            else
    ...
    {
        if(rZYCur.typ==rN)
            qubitVector.rotation(RAD*(rZYCur.ang-iter),rZYCur.typ,rZYCur.naX,
                                                                  rZYCur.naY,
                                                                  rZYCur.naZ);
        else
