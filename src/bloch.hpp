...
public:
    QBVector(float a_= 1.0, comp_d b_= 
                            comp_d(0.0,0.0));
    ...
    void rotation(float ng,rotType rt,
...
class Bloch : public QGLWidget
{
    ...
    struct rZYStruct
    {
        ...
        float naX, naY,naZ;
        QString msg;
        rZYStruct(float a=0,rotType t=rX,QString ms="",
             ...
             naX(ax),naY(ay),naZ(az)
        {
            ...
            switch(t)
            {
            case rX: msg+=QString("Rx(%1)").arg(okr(a)); 
                     break;
            case rY: msg+=QString("Ry(%1)").arg(okr(a)); 
                     break;
            case rZ: msg+=QString("Rz(%1)").arg(okr(a)); 
                     break;
            case rN: msg+=QString("Rn(%1); n(%2;%3;%4)")
                          .arg(okr(a)).arg(okr(ax,100)).arg(okr(ay,100))
                                                       .arg(okr(az,100)); 
                     break;
            }
        }
...
public:
    ...
    void setAB(float a=0, comp_d b=
                          comp_d(0,0));
    void setRandomPsi();
    void rotateZY(float zb, float yg, float zd, 
                                 QString msg=""); // ZY-вращение
    void rotateZX(float zb, float yx, float zd, 
                                 QString msg="");
    void rotateXY(float xb, float yg, float xd, 
                                 QString msg="");
    void setNewAxis(float ax, float ay, float az);
    void rotateN(float nt,QString msg="");
    void hideNewAx(){showNewAx=false;updateGL();}
    void reset();
    QBVector *getQBV(){return &qubitVector;}
